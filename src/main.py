...
def calc_mndwi_and_vectorize():
    ...
    # Define bounding box geometry for Boqueirão reservoir
    bbox = ee.Geometry.Rectangle([-36.21, -7.53, -36.05, -7.42])
    # Load Sentinel-2 SR collection for 10 years, filtered by bounding box
    ...
    # Vectorize water body using bounding box as geometry
    ...
    def get_largest(feats):
        ...
        max_area = 0
        for f in feats:
            area = ee.Feature(f).geometry().area(errorMargin=10).getInfo()
            if area > max_area:
                largest = f
                max_area = area
        return largest
